window.addEventListener("DOMContentLoaded", () => {
    ...
    // 게시판 및 인서트
    // 새 글 등록 버튼 누를 시 작성 창 출력
    const newdoc = document.querySelector(".containerFooter-newdoc")
    const insertscreen = document.querySelector(".boardInsert-wrapper")
    const boardlist = document.querySelector(".containerFooter-wrapper")
    if (newdoc != null) {
        newdoc.addEventListener("click",() =>{
            insertscreen.style.display = "flex";
            boardlist.style.display = "none";
        })
    }
    // 작성 창에서 취소 버튼 누를시 다시 게시판 출력
    const insertcancelbtn = document.querySelector(".boardInsert-cancel")
    if (insertcancelbtn != null) {
